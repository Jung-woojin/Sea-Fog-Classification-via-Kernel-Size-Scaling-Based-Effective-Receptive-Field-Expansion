...
Grad-CAM analysis for ERF experiment models.

Purpose:
    ERF 실험에 사용한 base/expanded 모델들이 실제 테스트 이미지에서
    어떤 영역을 보고 판단하는지 Grad-CAM으로 확인.

Steps:
    step1:
        동일 test image manifest 생성
        port별 normal / lowvis / seafog 각 50장 샘플링

    step2:
        각 모델별 Grad-CAM 생성
        prediction / correct / cam path CSV 저장
        기본적으로 전체 50/class 이미지를 전부 저장

    step3:
        base vs expanded Grad-CAM 비교 grid 생성
        class별 5장씩 논문 그림용 저장

Examples:
    python gradcam_erf_models.py --step all \
      --port daesan --port yeosu --port haeundae \
      --num_per_class 50 --viz_per_class 5
    ...
    python gradcam_erf_models.py --step step2 \
      --port yeosu \
      --backbone convnext \
      --target pred
"""
